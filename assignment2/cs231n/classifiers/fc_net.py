...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        dx1, grads['W2'], grads['b2'] = affine_backward(dout, cache2)   # dx1 is gradient with respect to X1
        _, grads['W1'], grads['b1'] = affine_relu_backward(dx1, cache1)
        grads['W2'] += self.reg*W2                                      # regularization term gradient
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers-1):  # for every layer except the last one
            ...
            caches.append(cache)
        w = self.params['W'+str(self.num_layers)]   # last layer
        ...
        n = self.num_layers
        da, grads['W'+str(n)], grads['b'+str(n)] = affine_backward(dout, caches[n-1]) # last layer
        grads['W'+str(n)] += self.reg*self.params['W'+str(n)]
        for i in range(n-1, 0, -1):
            da, grads['W'+str(i)], grads['b'+str(i)], dgamma, dbeta = affine_norm_relu_backward(da, caches[i-1], self.normalization, self.use_dropout)
            if self.normalization in ('batchnorm', 'layernorm'):
