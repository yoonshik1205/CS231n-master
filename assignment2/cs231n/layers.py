...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        cache = (x, mean, var, gamma, new_x, layernorm)
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
    elif mode == 'test':
...
def batchnorm_backward(dout, cache):
    ...
    # might prove to be helpful.                                              #
    ###########################################################################
    x, mean, var, gamma, new_x, axis = cache
    std = np.sqrt(var)

    # referencing the linked paper
    d_newx = dout*gamma
    dvar = -0.5*np.sum(d_newx*(x-mean), axis=0)/(std**3)
    dmean = -np.sum(d_newx, axis=0)/std - 2*dvar*np.sum(x-mean, axis=0)/dout.shape[0]

    dx = d_newx/std + dvar*2*(x-mean)/dout.shape[0] + dmean/dout.shape[0]
    dgamma = np.sum(dout*new_x, axis=axis)
...
def batchnorm_backward_alt(dout, cache):
    ...
    ###########################################################################
    _, _, var, gamma, new_x, axis = cache

    dx = gamma*(dout - (dout.sum(axis=0) + new_x*np.sum(new_x*dout, axis=0))/dout.shape[0])/np.sqrt(var)
    dgamma = np.sum(dout*new_x, axis=axis)
    dbeta = np.sum(dout, axis=axis)
    ###########################################################################
    #                             END OF YOUR CODE                            #
...
def dropout_forward(x, dropout_param):
    ...
    if mode == 'train':
        ...
        #######################################################################
        mask = np.random.choice((0, 1), p=(1-p, p), size=x.shape)/p
        out = x*mask
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    assert (H + 2*pad - HH)%stride==0, 'height wrong'
    assert (W + 2*pad - WW)%stride==0, 'width wrong'
    ...
    w_cols = w.reshape(F, -1).T
    positions = [(i*stride, j*stride) for i in range(outH) for j in range(outW)]
    for i, j in positions:
        x_rows = x_pad[:, :, i:i+HH, j:j+WW].reshape(N, -1)
        out[:, :, i//stride, j//stride] = x_rows @ w_cols + b
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def conv_backward_naive(dout, cache):
    ...
    positions = [(i, j) for i in range(outH) for j in range(outW)]
    for i, j in positions:
        xi, xj = i*stride, j*stride
        x_pad_rows = x_pad[:, :, xi:xi+HH, xj:xj+WW].reshape(N, -1)
        dx_pad[:, :, xi:xi+HH, xj:xj+WW] += (dout[:, :, i, j] @ w_rows).reshape(N, C, HH, WW)
        dw += (dout[:, :, i, j].T @ x_pad_rows).reshape(F, C, HH, WW)
...
def max_pool_forward_naive(x, pool_param):
    ...
    assert (H - PH)%stride==0, 'height wrong'
    assert (W - PW)%stride==0, 'width wrong'
    ...
    positions = [(i*stride, j*stride) for i in range(outH) for j in range(outW)]
    for i, j in positions:
        out[:, :, i//stride, j//stride] = x[:, :, i:i+PH, j:j+PW].max(axis=(2, 3))
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, pool_param)
...
def max_pool_backward_naive(dout, cache):
    ...
    for i, j in positions:
        xi, xj = i*stride, j*stride
        x_vals = x[:, :, xi:xi+PH, xj:xj+PW].reshape(N, C, -1)
        indices = x_vals.argmax(axis=2)
        for ni in range(N):
            for ci in range(C):
                dx[ni, ci, xi+indices[ni, ci]//PW, xj+indices[ni, ci]%PW] += dout[ni, ci, i, j]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx
